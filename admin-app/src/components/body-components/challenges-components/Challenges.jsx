...
const Challenges = ({ isDark }) => {
    ...
    const [editedExpectedOutput, setEditedExpectedOutput] = useState('');
    ...
    const openEditModal = (challenge) => {
        ...
        setEditedExpectedOutput(challenge.expectedOutput);
    ...
    const updateChallenge = async () => {
        ...
        if (!editedTitle || !editedDescription || !editedCriteria || !editedTimerDurationInMinutes || editedDifficultyLevel === 'Select Difficulty Level' || !editedExpectedOutput) {
        ...
        try {
            ...
            const updatedChallenge = {
                ...
                expectedOutput: editedExpectedOutput,
            };
    ...
    return (
        ...
        <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-evenly' }}>
                    ...
                    {challenges.map((challenge) => (
                        ...
                        <Card key={challenge._id} style={{ marginBottom: '16px', padding: '16px', boxShadow: '0px 2px 4px rgba(0, 0, 0, 0.1)', borderRadius: '8px', textAlign: 'center', width: '75vh', display: 'flex', flexDirection: 'column' }}>
                            ...
                            <p style={{ fontSize: '16px', marginBottom: '5px', fontWeight: 'bold' }}>Expected Output</p>
                            <p style={{ fontSize: '16px', marginBottom: '10px' }}>{challenge.expectedOutput}</p>
            ...
            >
                ...
                <div className={`input-container ${editedExpectedOutput ? 'active-label' : ''}`}>
                    <input
                        className='primary-form-element'
                        type="text"
                        id="challengeExpectedOutput"
                        name="challengeExpectedOutput"
                        onFocus={handleInputFocus}
                        onBlur={handleInputBlur}
                        value={editedExpectedOutput}
                        onChange={(event) => setEditedExpectedOutput(event.target.value)}
                        required
                    />
                    <label className={`primary-form-element ${isDark ? 'dark' : 'light'} ${editedExpectedOutput ? 'active-primary-form-label' : ''}`} htmlFor="challengeExpectedOutput">Expected Output</label>
                </div><br />
